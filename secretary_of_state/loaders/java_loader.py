...
class JavaLoader(BaseLoader):
    def load(self) -> MachineDescription:
        ...
        config_str = regex.sub(r"\/\/.*$\n", "", config_str, flags=regex.MULTILINE)

        # Trim the `new StateMachineConfiguration` line
        config_str = regex.sub(
            r"\S+.*=.*new StateMachineConfiguration.*;", "", config_str
        )

        # Trim the `new StateMachine` line
        config_str = regex.sub(
            r"^.*new StateMachine$", "", config_str, flags=regex.MULTILINE
        )
        ...
        state_type_match = regex.search(r"\.configure\((.*)\..*\)", config_str)
        ...
        trigger_type_match = regex.search(r"\.permit(?:If)?\((\w+)", config_str)
        ...
        config_str = regex.sub(r"\.configure\((\S+)\)", r"\1 = [\n", config_str)
        ...
        while (
            ...
            new_text = regex.sub(r"\s+", " ", new_text)
        ...
        config_str = regex.sub(r"^\s*(\S)", r"\1", config_str, flags=regex.MULTILINE)
